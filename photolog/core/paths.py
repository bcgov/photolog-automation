...
def effective_config() -> MachineConfig:
    ...
    return MachineConfig(
        ...
        gm_exe=gm_exe or "",
        notify_sender=cfg.notify_sender,
        notify_recipient=cfg.notify_recipient,
    )
...
def cleanup_orphaned_files() -> None:
    ...
    def scan_and_clean(root: Path) -> None:
        ...
        try:
            for year_dir in root.iterdir():
                if not year_dir.is_dir() or not _YEAR_RE.fullmatch(year_dir.name):
                    continue

                # Clean .tmp files (stale manifest writes)
                for tmp_file in year_dir.glob("*.tmp"):
                    try:
                        tmp_file.unlink()
                    except OSError as e:
    ...
    cfg = effective_config()
    if cfg.hr_root:
        scan_and_clean(Path(cfg.hr_root))
    if cfg.tn_root:
        scan_and_clean(Path(cfg.tn_root))
